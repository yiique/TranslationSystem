...
int AsyncNetDriver::PostEvent(const Event & e)
{
    ...
    //与post_handler函数进行bind,根据不同的事件类型执行不同的req_*函数
...
{
    //新建一个acceptor,插入netdriver的acceptorManager属性,并启动
    if( !p_edata || !p_caller )
    {
...
{
    //新建一个connection,插入netdriver的connectionManager属性,并启动
    if( !p_edata || !p_caller )
    {
        lerr << "ERROR: INPUT is null. " << endl;
        return;
...
{
    //根据caller找到对应线程的句柄,执行该线程的send操作
    if( !p_edata || !p_caller )
    {
        lerr << "ERROR: INPUT is null." << endl;
        return;
