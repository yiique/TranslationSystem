...
#ifndef PROJECT_DBOPERATION_H
#define PROJECT_DBOPERATION_H
...
public:
    ...
    static int LoadAllDict(vector<DictInfo> & dict_info_vec);           // 读取全部dict_info
    static int LoadAllWord(vector<WordInfo> & word_info_vec);           // 读取全部word_info

    static int ModifyImportStatus(const vector<size_t> & import_word_id);   // import 状态,但实际上没有这个表(应该被废弃)
