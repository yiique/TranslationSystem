...
bool DBConnPool::Initialize(const string &db_name, const string &db_ip, const string &db_usr, const string &db_pwd,
                            const int db_port, const int db_conn_num)
{
    m_db_name = db_name;
    m_db_ip = db_ip;
    m_db_usr = db_usr;
    ...
    if (m_db_conn_num <= 0)
    {
        lerr << "DB connection pool number can't set to : " << m_db_conn_num << endl;
        return false;
    }
